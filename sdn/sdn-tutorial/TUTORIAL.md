...
```
                 ┌────────────────────┐
                 │   ctrl (Ryu)       │   OpenFlow / TCP 6653
                 │   172.20.20.10     │◄───────────────┐
                 └────────────────────┘                │ (control plane,
                                                        │  over mgmt net)
                 ┌────────────────────┐                │
   h1 ───eth1────┤        s1          ├────────────────┘
 10.0.0.1        │   Open vSwitch     │
   h2 ───eth2────┤        br0         │   (data plane)
 10.0.0.2        │  OpenFlow 1.3      │
   h3 ───eth3────┤                    │
 10.0.0.3        └────────────────────┘
```
...
**Controller terminal:**
```bash
docker exec -it clab-sdn-ctrl ryu-manager /apps/hub.py
```
...
**Controller terminal:**
```bash
docker exec -it clab-sdn-ctrl ryu-manager /apps/firewall.py
```
